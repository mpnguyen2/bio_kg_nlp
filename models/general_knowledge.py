...
class KnowledgeEnhancerModule(nn.Module):
    def __init__(self, configs, common_embs={}):
        ...
        # Dictionary from common concepts to learned relational embeddings:
        self.common_embs = common_embs
        self.common_embs_size = 1 if len(common_embs)==0 else self.common_embs[next(iter(self.common_embs))].shape[0]
        ...
        # Feed forward from commonly learned embedding to embedding of the same dim with new concepts
        self.common_linear = FFNNModule(input_size = self.common_embs_size,
                                        hidden_sizes = [],
                                        output_size = UMLS_EMBS_SIZE,
                                        dropout = configs['dropout_rate'])
    ...
    def forward(self, text, ie_preds):
        ...
        # Note this graph avoid connected edges btw cuids in common_embs
        ...
        initial_node_embs_data = [self.common_linear(self.common_embs[n]) if n in self.common_embs
            else self.cuid2embs[n] for n in nodes]
        initial_node_embs = torch.tensor(np.array(initial_node_embs_data, dtype=np.float), dtype=torch.float).to(self.device)
